<template>
  <div>
    ...
    <Modal v-model="modal1" title="物流信息">
      ...
      <p v-if="tracesList.length == 0">暂无物流信息</p>
      <Timeline v-else>
        <TimelineItem v-for="(item, index) in tracesList" :key="index">
...
<script>
  import { orderlist, logistics, orderSend, logisticsFirmTrack, orderSendupdate } from '@/api/order'
  export default {
    name: 'discountCoupon',
    data() {
      /* 状态 */
      ...
      return {
        ...
        tableColumns: [
          ...
          },
          {
            ...
            render: (h, p) => {
              return h('div', [
                h('span', {}, p.row.type == 'LOCAL' ? '--' : p.row.outsideOrderNo)
              ])
            }
          },
          {
            ...
            render: (h, p) => {
              return h('div', [
                h('span', {}, p.row.type == 'LOCAL' ? '--' : p.row.outsideStoreNo)
              ])
            }
          },
          {
              ...
              return h('div', [
                ...
                h('Button', {
                  props: {
                    type: 'primary',
                    size: 'small',
                    disabled: row.type == 'LOCAL' ? false : true
                  },
                  style: {
                    marginRight: '5px',
                    display: row.orderStatus == 'ALREADY_SHIP' ? 'block' : 'none'
                  ...
                  on: {
                    click: () => {
                      this.orderSendBtnupdate(row.id, row)
                    }
                  }
                }, '修改发货')
